...
export function argumentValue(argv: string[], name: string): string | null {
...
type ManifestEntry = { count: number; fingerprint: string };
export type ResetManifest = Record<string, ManifestEntry>;
...
export function resetSnapshotToken(snapshot: unknown): string {
  return createHash('sha256')
    .update(JSON.stringify(stable(snapshot)))
    .digest('hex');
}

export function compareResetManifest(
  expected: ResetManifest,
  current: ResetManifest,
): string[] {
  const tables = [
...
export function partitionInventoryTables(tableNames: string[]): {
  known: string[];
  unknown: string[];
} {
  const knownSet = new Set<string>(BETANO_DIARIO_RESET_TABLES);
  return {
...
export function buildSheetControlRanges(
  sheetTitle: string,
  headers: string[],
  lastRow: number,
): string[] {
  ...
  return ['STATUS', 'E-MAIL'].map((required) => {
    ...
    return `${quoteSheetTitle(sheetTitle)}!${column}2:${column}${lastRow}`;
  });
}
