...
import {
  BETNACIONAL_MARKED_STATUS,
  SheetRow,
} from '../domain/betnacional.types.js';

@Injectable()
export class BetnacionalSheetService implements OnModuleInit {
  private readonly logger = new Logger(BetnacionalSheetService.name);
  ...
  onModuleInit() {
    this.sheetId = this.config.getOrThrow<string>('BETNACIONAL_SHEET_ID');
    this.sheetTab = this.config.getOrThrow<string>('BETNACIONAL_SHEET_TAB');
  ...
  async markRowUsed(rowIndex: number, userEmail: string): Promise<void> {
    ...
    await this.sheets.spreadsheets.values.update({
      ...
      requestBody: { values: [[userEmail, BETNACIONAL_MARKED_STATUS]] },
    });
  }
    ...
    await this.sheets.spreadsheets.values.batchUpdate({
      ...
      requestBody: {
        ...
        data: entries.map((e) => ({
          ...
          values: [[e.email, BETNACIONAL_MARKED_STATUS]],
        })),
      },
    });
  }
