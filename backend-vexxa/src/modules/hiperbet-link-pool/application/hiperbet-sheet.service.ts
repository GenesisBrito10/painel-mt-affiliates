...
import { HIPERBET_MARKED_STATUS, SheetRow } from '../domain/hiperbet.types.js';

@Injectable()
export class HiperbetSheetService implements OnModuleInit {
  private readonly logger = new Logger(HiperbetSheetService.name);
  ...
  onModuleInit() {
    this.sheetId = this.config.getOrThrow<string>('HIPERBET_SHEET_ID');
    this.sheetTab = this.config.getOrThrow<string>('HIPERBET_SHEET_TAB');
  ...
  async markRowUsed(rowIndex: number, userEmail: string): Promise<void> {
    ...
    await this.sheets.spreadsheets.values.update({
      ...
      requestBody: { values: [[userEmail, HIPERBET_MARKED_STATUS]] },
    });
  }
    ...
    await this.sheets.spreadsheets.values.batchUpdate({
      ...
      requestBody: {
        ...
        data: entries.map((e) => ({
          ...
          values: [[e.email, HIPERBET_MARKED_STATUS]],
        })),
      },
    });
  }
