...
const DEFAULT_SHEET_ID = '14HvBUvAuzZy96r9PyTfiVNeuUXcKTox7KS7NbfH84AQ';
const DEFAULT_SHEET_TAB = 'Historico';

@Injectable()
export class HiperbetMetricsSheetService implements OnModuleInit {
  private readonly logger = new Logger(HiperbetMetricsSheetService.name);
  ...
  onModuleInit() {
    this.sheetId = this.config.get<string>(
      'HIPERBET_METRICS_SHEET_ID',
      DEFAULT_SHEET_ID,
    );
    this.sheetTab = this.config.get<string>(
      'HIPERBET_METRICS_SHEET_TAB',
      DEFAULT_SHEET_TAB,
    );
