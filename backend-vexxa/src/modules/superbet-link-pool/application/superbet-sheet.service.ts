...
import { SheetRow, SUPERBET_MARKED_STATUS } from '../domain/superbet.types.js';

@Injectable()
export class SuperbetSheetService implements OnModuleInit {
  private readonly logger = new Logger(SuperbetSheetService.name);
  ...
  onModuleInit() {
    this.sheetId = this.config.getOrThrow<string>('SUPERBET_SHEET_ID');
    this.sheetTab = this.config.getOrThrow<string>('SUPERBET_SHEET_TAB');
  ...
  async markRowUsed(rowIndex: number, userEmail: string): Promise<void> {
    ...
    await this.sheets.spreadsheets.values.update({
      ...
      requestBody: { values: [[SUPERBET_MARKED_STATUS, userEmail]] },
    });
  }
    ...
    await this.sheets.spreadsheets.values.batchUpdate({
      ...
      requestBody: {
        ...
        data: entries.map((e) => ({
          ...
          values: [[SUPERBET_MARKED_STATUS, e.email]],
        })),
      },
    });
  }
