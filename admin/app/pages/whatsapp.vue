...
// ─── Grupos de destino (multi-grupo) ────────────────────────────────────────
// Grupos disponíveis que ainda NÃO estão salvos (evita repetição na UI).
const addableGroupItems = computed(() => {
  const savedIds = new Set(targetGroups.value.map((g) => g.groupId))
  return groups.value
    .filter((g) => !savedIds.has(g.id))
    .map((g) => ({ label: g.name || g.id, value: g.id }))
})

async function loadTargetGroups() {
  try {
    targetGroups.value = await wa.fetchTargetGroups()
  } catch {
    /* ignore — exibido vazio */
  }
}

async function doAddGroup() {
  const id = groupToAdd.value
  if (!id) {
    toast.add({ title: 'Selecione um grupo para adicionar', color: 'warning' })
    return
  }
  if (targetGroups.value.some((g) => g.groupId === id)) {
    toast.add({ title: 'Grupo já adicionado', color: 'warning' })
    return
  }
  addingGroup.value = true
  try {
    const name = groups.value.find((g) => g.id === id)?.name ?? ''
    await wa.addTargetGroup({ groupId: id, name })
    await loadTargetGroups()
    groupToAdd.value = undefined
    toast.add({ title: 'Grupo adicionado', color: 'success', icon: 'i-lucide-check-circle' })
  } catch (e) {
    const msg = (e as { data?: { message?: string } })?.data?.message ?? 'Erro ao adicionar grupo'
...
async function doRemoveGroup(groupId: string) {
  removingGroupId.value = groupId
  try {
    await wa.removeTargetGroup(groupId)
    await loadTargetGroups()
    toast.add({ title: 'Grupo removido', color: 'success', icon: 'i-lucide-trash-2' })
  } catch {
    toast.add({ title: 'Erro ao remover grupo', color: 'error' })
  } finally {
    removingGroupId.value = null
  }
}

async function doTestGroup(groupId: string) {
  testingGroupId.value = groupId
  try {
    await wa.testGroup(groupId)
...
async function saveSettings() {
  ...
  try {
    const updated = await wa.saveSettings({
      ...
      linkPoolAlertNumber: settings.value.linkPoolAlertNumber,
      linkPoolAlertThreshold: settings.value.linkPoolAlertThreshold,
      linkPoolAlertEnabled: settings.value.linkPoolAlertEnabled,
    })
...
async function doTestNumber() {
  const number = settings.value?.linkPoolAlertNumber?.trim()
  if (!number) {
    toast.add({ title: 'Informe um número primeiro', color: 'warning' })
    return
  }
  testingNumber.value = true
  try {
    const res = await wa.testNumber(number)
    if (res.valid && res.sent) {
      toast.add({ title: 'Número válido — mensagem de teste enviada', color: 'success', icon: 'i-lucide-check-circle' })
    } else if (res.valid) {
      toast.add({ title: 'Número válido, mas o envio falhou', color: 'warning' })
    } else {
      toast.add({ title: 'Número não encontrado no WhatsApp', color: 'error', icon: 'i-lucide-x-circle' })
    }
  } catch (e) {
    const msg = (e as { data?: { message?: string } })?.data?.message ?? 'Erro ao testar número'
    toast.add({ title: msg, color: 'error' })
  } finally {
    testingNumber.value = false
  }
}
    ...
    <UCard>
      <template #header>
        <div class="flex items-center justify-between gap-2">
          <h2 class="font-semibold">Grupos de destino</h2>
          <span class="text-xs text-gray-500">{{ targetGroups.length }} grupo(s) — o mesmo comprovante é enviado a todos</span>
        </div>
      </template>
      ...
      <div class="flex flex-col sm:flex-row gap-3 sm:items-end">
        <div class="flex-1">
          <label class="text-sm text-gray-500">Adicionar grupo / comunidade</label>
          <USelectMenu
            v-model="groupToAdd"
            :items="addableGroupItems"
            value-key="value"
            label-key="label"
        ...
        <UButton icon="i-lucide-plus" :loading="addingGroup" :disabled="!groupToAdd" @click="doAddGroup">Adicionar</UButton>
      </div>

      <!-- Lista de grupos salvos -->
      <div class="mt-4 divide-y divide-gray-100 dark:divide-gray-800">
        <p v-if="targetGroups.length === 0" class="text-sm text-gray-500 py-3">
          Nenhum grupo salvo. Adicione ao menos um grupo para enviar comprovantes.
        </p>
        <div
        ...
        >
          <img
            ...
            @error="g.pictureUrl = null"
          >
          <Avatar v-else :name="g.name || g.groupId" :size="40" />
          <div class="flex-1 min-w-0">
            <p class="font-medium truncate">{{ g.name || g.groupId }}</p>
            <p class="text-xs text-gray-500 truncate">{{ g.groupId }}</p>
          </div>
          <UButton
            size="xs"
            color="primary"
            variant="soft"
            icon="i-lucide-send"
            :loading="testingGroupId === g.groupId"
            @click="doTestGroup(g.groupId)"
          >Enviar teste</UButton>
          <UButton
            size="xs"
            color="error"
            variant="ghost"
            icon="i-lucide-x"
            :loading="removingGroupId === g.groupId"
            :title="'Remover grupo'"
            @click="doRemoveGroup(g.groupId)"
          />
        </div>
      </div>
    </UCard>
    ...
    <UCard v-if="settings">
      <template #header>
        <div class="flex items-center gap-2">
          ...
          <h2 class="font-semibold">Alerta de planilha de links</h2>
        </div>
      </template>
      <p class="text-sm text-gray-500 mb-4">
        Notifica por WhatsApp (no número abaixo) e por push (admins) — uma única
        vez — quando os links livres de uma casa caírem até o limite. O alerta
        re-arma sozinho quando a planilha é reabastecida.
      </p>
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="md:col-span-2">
          <label class="text-sm text-gray-500">Número de alerta (com DDI/DDD)</label>
          <UInput
            v-model="settings.linkPoolAlertNumber"
            placeholder="5511999999999"
            icon="i-lucide-phone"
          />
        </div>
        <div>
          <label class="text-sm text-gray-500">Limite de links livres</label>
          <UInput v-model.number="settings.linkPoolAlertThreshold" type="number" :min="0" />
        </div>
      </div>
      <div class="flex items-center gap-4 mt-4">
        <USwitch v-model="settings.linkPoolAlertEnabled" label="Alerta ativo" />
      </div>
      <template #footer>
        <div class="flex flex-wrap gap-2">
          <UButton :loading="savingSettings" icon="i-lucide-save" @click="saveSettings">Salvar</UButton>
          <UButton
            color="neutral"
            variant="soft"
            icon="i-lucide-send"
            :loading="testingNumber"
            :disabled="!settings.linkPoolAlertNumber"
            @click="doTestNumber"
          >
            Testar número
          </UButton>
